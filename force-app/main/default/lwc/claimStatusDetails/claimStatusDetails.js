...
export default class Datatable extends LightningElement {
    ...
    @api selectedRowClaimId = '';
    @api selectedRowClaimType = '';
    @api selectedRowVendorId = '';
    
    @api selectedRowSource = '';
    ...
    get hideCheckboxColumn() {
        ...
        return (this.cb_hideCheckboxColumn == CB_TRUE) ? false : true;
    }
    ...
    connectedCallback() {
        ...
        if (this.isUserDefinedObject) {
            ...
            if(this.tableDataString.indexOf('"NotFound"') == -1)
            {
               _tableData = JSON.parse(this.tableDataString);
                for(var i = 0; i < _tableData.length; i++)
				{
					var data = _tableData[i];
					_tableData[i]['dataSource'] = 'CSC'; 
					var keyField = {claimId:  data.claimID, dateOfService: new Date(data.dateofService.split('T')[0]), source: data.dataSource, claimType: data.claimType };
					_tableData[i]['keyField'] = JSON.stringify(keyField); 
					if(_tableData[i]['claimType'] == '0')
					{
						_tableData[i]['claimType'] = 'Medical';
					}
					else {
						_tableData[i]['claimType'] = 'Behavior';
					}
					var dateofService = new Date(_tableData[i]['dateofService'].slice(0, 10));
					dateofService.setDate(dateofService.getDate() + 1);
					_tableData[i]['dateofService'] = dateofService;
				}
            } 
            if(typeof this.columnCellAttribs === 'string')
            {
                if(this.columnCellAttribs.indexOf('"NotFound"') == -1)
                {
					var qnextData = JSON.parse(this.columnCellAttribs);
					for(var i = 0; i < qnextData.length; i++)
					{
						qnextData[i]['dataSource'] = 'QNXT';
						var data = qnextData[i];
						var keyField = {claimId:  data.claimID, dateOfService: new Date(data.dateofService.split('T')[0]), source: data.dataSource, claimType: data.claimType };
						qnextData[i]['keyField'] = JSON.stringify(keyField); 
						if(qnextData[i]['claimType'] == '0')
						{
							qnextData[i]['claimType'] = 'Medical';
						}
						else {
							qnextData[i]['claimType'] = 'Behavior';
						}
						var dateofService = new Date(qnextData[i]['dateofService'].slice(0, 10));
						dateofService.setDate(dateofService.getDate() + 1);
						qnextData[i]['dateofService'] = dateofService;
						_tableData.push(qnextData[i]);
					}
            ...
            this._tableData = JSON.parse(JSON.stringify(_tableData));
        ...
        if (this.isUserDefinedObject && !this.isConfigMode) {
          ...
          this.columns.splice(0, 0, {
                ...
                filterAttrib.actions = [
                        {label: this.label.SetFilterAction, disabled: false, name: 'filter_0', iconName: 'utility:filter'},
                        {label: this.label.ClearFilterAction, disabled: true, name: 'clear_0' + columnNumber, iconName: 'utility:clear'}
                    ];*/
            }
        });
        ...
        this.basicColumns.forEach(colDef => {
            ...
            switch (this.filterAttribType) {
                case 'cols':
                    ...
                    break;
                case 'all':
                    var dummyColumn = columnNumber + 1;
                    filterAttrib.column = dummyColumn; 
                    filterAttrib.filter = true;
                    filterAttrib.actions = [
                        {label: this.label.SetFilterAction, disabled: false, name: 'filter_' + dummyColumn, iconName: 'utility:filter'},
                        {label: this.label.ClearFilterAction, disabled: true, name: 'clear_' + dummyColumn, iconName: 'utility:clear'}
                    ]; 
                    break;
                default:
                    filterAttrib.filter = false;
    ...
    handleRowAction(event) {
        ...
        // let currentSelectedRows = mydata;
		...
		//this.outputSelectedRows = [...rowData]; 
        this.outputSelectedRows = [selectedrow]; 
        this.dispatchEvent(new FlowAttributeChangeEvent('outputSelectedRows', this.outputSelectedRows));
...
// TODO - Add support for User Defined Validation Rules
    ...
    handleRowSelection(event) {
        ...
        console.log('In row selection'+ event.detail.selectedRows);
        try{
        ...
        /*this.flowInputVariables = {"claimId":  this.outputSelectedRows[0].claimID, "dateOfService": new Date(this.outputSelectedRows[0].dateofService.split('T')[0]), "source": this.outputSelectedRows[0].dataSource, "memberId": this.columnIcons, claimType: this.outputSelectedRows[0].claimType };
        var detailForm = this.template.querySelector("c-claim-status-details-form");
        detailForm.flowInputData = this.flowInputVariables;
        detailForm.runFlow();
        */
    ...
    updateNumberOfRowsSelected(currentSelectedRows) {
        ...
        if((this.numberOfRowsSelected == 1 && (this._tableData.length == 1 || this.singleRowSelection))) {
            ...
            this.selectedRowClaimId = this.outputSelectedRow['claimID'];
            if(this.outputSelectedRow['claimType'] == 'Medical')
            {
                this.outputSelectedRow['claimType'] = 0;
            }
            else{
                this.outputSelectedRow['claimType'] = 1;
            }
            this.selectedRowClaimType = this.outputSelectedRow['claimType'];

            this.selectedRowSource = this.outputSelectedRow['dataSource'];
            
            if (this.selectedRowSource == 'CSC')
            {
                this.selectedRowVendorId = this.outputSelectedRow['vendorId'];
            }
                            ...
                            if (this.columnFilterValues[col] && this.columnFilterValues[col] != null) {
                                ...
                                switch(this.filterColumns[col].type) {
                                    ...
                                    case 'time':
                                        ...
                                        let dt = new Date(row[fieldName].slice(0,10));
                                        let filteredDate = new Date(this.columnFilterValues[col]);
                                        filteredDate.setDate(filteredDate.getDate() + 1);
                                        // dt.setDate(dt.getDate() + -1);
                                        // var d1 = dt.slice(0,10);
                                        if (filteredDate.getTime() != dt.getTime()) {    // Check for date match on date & time fields
                                            match = false;
                                            break;                                
                                        }
                                        break;
...
/*      // Validate Edited Rows
    ...
    handleClick(event)
    {
        ...
        //alert(rowNode.dataset.rowKeyValue);
    }
}